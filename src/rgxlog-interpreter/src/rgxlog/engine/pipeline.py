...
def assert_correct_node(tree, node_name, len_children=None, *children_names):
    assert tree.data == node_name, \
        f"bad node name: {node_name}\n" \
        f"actual node name: {tree.data}"

    if len_children is not None:
        assert len(tree.children) == len_children, \
            f"bad children length: {len_children}\n" \
            f"actual children length: {len(tree.children)}"

    if children_names is not None:
        for idx, name in enumerate(children_names):
            assert tree.children[idx].data == name, \
                f"bad child name at index {idx}: {name}\n" \
                f"actual child name: {tree.children[idx].data}"
