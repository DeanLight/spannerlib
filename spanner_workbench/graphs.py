...
PRETTY_INDENT = " " * 4
ROOT_NODE_ID = "__spanner_workbench_root"
ROOT_TYPE = "root"
TYPE = "type"
STATE = "state"
VALUE = "value"
...
class DependencyGraph(NetxGraph):
    """
    The `DependencyGraph` class is designed to map and manage dependencies between rule relations in an spanner_workbench program. Each rule relation in the program corresponds to a node in this graph, with the node's ID being the name of the rule relation.

    ### Example

    Consider the following spanner_workbench program:
...
class TermGraph(TermGraphBase):
    """
        This class is designed to transform each rule node in an spanner_workbench program into an execution graph. These execution graphs are then added to a term graph. <br>
        ...
        Here is a sample spanner_workbench program for illustration:
        ...
        * **Base Relations**: Each base relation is directly connected to the `join_node`.
        ...
        Here's a representation of the term graph for the spanner_workbench program discussed:
