...
def naive_execution(parse_graph: GraphBase, # a parse graph to execute
                    ...
                    spanner_workbench_engine: spanner_workbenchEngineBase # a spanner_workbench engine that will be used to execute the term graph
    ...
    def compute_rule(relation_name: str, do_reset: bool = True) -> None:
        ...
        spanner_workbench_engine.clear_tables(mutually_recursive)
        ...
        while not fixed_point:
            ...
            for relation in mutually_recursive:
                ...
                visited_nodes = set()
                initial_len = spanner_workbench_engine.get_table_len(current_computed_relation)
                compute_postorder(current_computed_relation)
                is_stopped = spanner_workbench_engine.get_table_len(current_computed_relation) == initial_len
    ...
    def compute_node(node_id: GraphBase.NodeIdType) -> None:
        ...
        term_type_to_engine_op: Dict[TermNodeType, Callable] = {
            TermNodeType.RULE_REL: spanner_workbench_engine.operator_copy,
            TermNodeType.UNION: spanner_workbench_engine.operator_union,
            TermNodeType.JOIN: spanner_workbench_engine.operator_join,
            TermNodeType.PROJECT: spanner_workbench_engine.operator_project,
            TermNodeType.SELECT: spanner_workbench_engine.operator_select
        }
        ...
        if term_type is TermNodeType.GET_REL:
            ...
            output_relation = spanner_workbench_engine.compute_ie_relation(ie_rel_in, ie_func_data, rel_in)
    ...
    node_type_to_action: Dict[Union[str, ParseNodeType], Callable] = {
        ParseNodeType.RULE: lambda rule_: spanner_workbench_engine.declare_relation_table(rule_.head_relation.as_relation_declaration()),
        ParseNodeType.RELATION_DECLARATION: spanner_workbench_engine.declare_relation_table,
        ParseNodeType.ADD_FACT: spanner_workbench_engine.add_fact,
        ParseNodeType.REMOVE_FACT: spanner_workbench_engine.remove_fact,
    ...
    for parse_id in parse_node_ids:
        ...
        if parse_node_type == ParseNodeType.QUERY:
            ...
            query_result = (query, spanner_workbench_engine.query(query))
